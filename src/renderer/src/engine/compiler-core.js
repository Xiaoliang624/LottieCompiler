...
function finitePoint(value) {
  ...
  if (value && typeof value === 'object') {
    const x = Number(value.x != null ? value.x : value[0]);
    const y = Number(value.y != null ? value.y : value[1]);
    return Number.isFinite(x) && Number.isFinite(y) ? [round(x), round(y)] : null;
  }
...
function finitePointList(value) {
  if (!Array.isArray(value)) {
    return [];
  }
...
function tangentList(value, count) {
  const points = finitePointList(value);
  const tangents = [];
  for (let i = 0; i < count; i++) {
    tangents.push(points[i] || [0, 0]);
  }
  return tangents;
}

function pathCoordinateSpace(value, animation) {
  const raw = value && typeof value === 'object'
    ? (value.coordinateSpace || value.space || animation.coordinateSpace || animation.space)
    : (animation.coordinateSpace || animation.space);
  return String(raw || 'local').toLowerCase().replace(/[\s_-]/g, '');
}

function localizePathVertices(vertices, coordinateSpace, node) {
  if (!node || coordinateSpace === 'local' || coordinateSpace === 'center' || coordinateSpace === 'relative') {
    return vertices;
  }

  const offset = coordinateSpace === 'absolute' || coordinateSpace === 'canvas' || coordinateSpace === 'scene'
    ? getNodeCenter(node)
    : { x: clampNumber(node.width, 0) / 2, y: clampNumber(node.height, 0) / 2 };

  return vertices.map((point) => [round(point[0] - offset.x), round(point[1] - offset.y)]);
}

function normalizeShapePathValue(value, node, animation) {
  if (Array.isArray(value) && value.length === 1 && value[0] && typeof value[0] === 'object' && Array.isArray(value[0].v)) {
    return normalizeShapePathValue(value[0], node, animation);
  }

  const coordinateSpace = pathCoordinateSpace(value, animation || {});
  if (typeof value === 'string') {
    const center = coordinateSpace === 'absolute' || coordinateSpace === 'canvas' || coordinateSpace === 'scene'
      ? getNodeCenter(node)
      : (coordinateSpace === 'topleft' || coordinateSpace === 'node'
        ? { x: clampNumber(node && node.width, 0) / 2, y: clampNumber(node && node.height, 0) / 2 }
        : { x: 0, y: 0 });
    const parsed = parseSvgPathData(value, center.x, center.y)[0];
    if (!parsed || parsed.points.length === 0) {
      return null;
    }
    return {
  ...
  let vertices = finitePointList(object.v || object.vertices || object.points || (Array.isArray(value) ? value : null));
  if (vertices.length === 0) {
    return null;
  }

  vertices = localizePathVertices(vertices, coordinateSpace, node);
  return {
    i: tangentList(object.i || object.inTangents || object.in || object.ti, vertices.length),
    o: tangentList(object.o || object.outTangents || object.out || object.to, vertices.length),
    v: vertices,
    c: typeof object.c === 'boolean' ? object.c : (typeof object.closed === 'boolean' ? object.closed : true)
  };
}

function samePathStructure(a, b) {
  return Boolean(
    a &&
      b &&
      Array.isArray(a.v) &&
...
function pathValueFromKeyframe(keyframe) {
  if (!keyframe || typeof keyframe !== 'object') {
    return null;
  }

  if (keyframe.value != null) return keyframe.value;
  if (keyframe.path != null) return keyframe.path;
  if (keyframe.shapePath != null) return keyframe.shapePath;
  if (keyframe.vertices != null) return keyframe.vertices;
  if (keyframe.points != null) return keyframe.points;
  if (keyframe.v != null || keyframe.i != null || keyframe.o != null || keyframe.c != null || keyframe.closed != null) {
    return keyframe;
  }
  return null;
}

function animatedPathProperty(animationsForNode, node) {
  const frames = [];
  let structure = null;

  for (const animation of animationsForNode || []) {
    if (normalizeAnimationProperty(animation.property) !== 'path' || !Array.isArray(animation.keyframes)) {
      continue;
    }

    for (const keyframe of animation.keyframes) {
      const rawValue = pathValueFromKeyframe(keyframe);
      const path = normalizeShapePathValue(rawValue, node, animation);
      if (!path) {
        continue;
      }
      if (!structure) {
      ...
      if (!samePathStructure(structure, path)) {
        continue;
      }
      frames.push(Object.assign({}, keyframe, { value: path }));
    }
  }

  return buildAnimatedProperty(frames, (value) => value);
}
...
function applyShapeAnimations(shapeItems, animationsForNode, node) {
  if (!animationsForNode || animationsForNode.length === 0) {
    return;
  }
  ...
  const shapePathItem = shapeItems.find((item) => item && item.ty === 'sh' && item.ks);
  if (shapePathItem) {
    const pathProperty = animatedPathProperty(animationsForNode, node);
    if (pathProperty) {
      shapePathItem.ks = pathProperty;
    }
  }
}
...
function createShapeLayer(node, index, animationsForNode, durationFrames, parentNode) {
  ...
  applyShapeAnimations(shapeItems, animationsForNode, node);
